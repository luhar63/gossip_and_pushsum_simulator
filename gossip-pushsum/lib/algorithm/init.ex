defmodule Project.Algorithm do
  def start(superPid, algorithm, startTime) do
    run_algorithm(superPid, algorithm, startTime)
  end

  def run_algorithm(superPid, algorithm, startTime) do
    cond do
      algorithm == "gossip" ->
        Project.Algorithm.Gossip.start(superPid, startTime)

      algorithm == "push-sum" ->
        Project.Algorithm.PushSum.start(superPid, startTime)
    end
  end
  ...
  def infiniteLoop(node_list, startTime, prevActivelist, invocationTime, timeout) do
    ...
    if(
      ...
      IO.puts("Timed out: node didn't send anything for last 20 sec")
      getSpread(node_list)
      IO.puts("Total Time:")
