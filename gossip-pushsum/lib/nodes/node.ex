defmodule Project.Nodes.Worker do
  ...
  # #################### GOSSIP ############################
  def sendMessage(toPid, message, _fromPid, startTime, superPid) do
  ...
  def resend_message(neighbors, message, pid, startTime, superPid) do
    ...
    if(length(neighbors) > 0) do
      ...
      Process.sleep(50)
      resend_message(neighbors, message, pid, startTime, superPid)
    end
  end
